...
class VideoSequenceTransform:
    """Minor data augmentation for video sequences of shape (num_frames, H, W, C)"""
    def __init__(self, noise_std=0.01, flip_prob=0.5, brightness_range=(0.95, 1.05), contrast_range=(0.95, 1.05)):
    ...
    def __call__(self, video: np.ndarray) -> np.ndarray:
        # video: (num_frames, H, W, C) or (batch, num_frames, H, W, C)
        if video.ndim == 5:
            # batch mode
            return np.stack([self._augment_single(v) for v in video], axis=0)
        else:
            return self._augment_single(video)

    def _augment_single(self, video: np.ndarray) -> np.ndarray:
        # Add Gaussian noise
        video = video + np.random.normal(0, self.noise_std, video.shape).astype(video.dtype)
        # Horizontal flip
        if np.random.rand() < self.flip_prob:
            video = np.flip(video, axis=2)  # Flip width axis
...
def get_video_training_transforms():
    """Get minor training transforms for video data"""
    return VideoSequenceTransform(
        noise_std=0.01,
        flip_prob=0.5,
...
def get_video_validation_transforms():
    """Get minimal validation transforms for video data (no augmentation)"""
    return VideoSequenceTransform(
        noise_std=0.0,
        flip_prob=0.0,

...
class PoseSequenceTransform:
    ...
    def __init__(self, transforms: List[str] = [], **kwargs):
        """
    ...
    def _temporal_jitter(self, sequence: np.ndarray) -> np.ndarray:
        ...
        if jitter_frames > 0:
            ...
            shifts = np.random.randint(-jitter_frames,
                                       jitter_frames + 1, num_frames)
            shifts = np.clip(shifts, -num_frames//3,
                             num_frames//3)  # Limit shifts
    ...
    def _temporal_interpolate(self, sequence: np.ndarray) -> np.ndarray:
        ...
        if num_frames > 4 and np.random.uniform(0, 1) > .5:
            ...
            interpolated = np.zeros(
                shape=(keep_frames, sequence.shape[1], sequence.shape[2]), dtype=np.float32)
    ...
    def _temporal_sample(self, sequence: np.ndarray) -> np.ndarray:
        ...
        if num_frames > 4 and np.random.uniform(0, 1) > .5:
            ...
            indices = np.sort(np.random.choice(
                num_frames, keep_frames, replace=False))
